  function [SPM] = spm_spm_vb(SPM) % VERSION: spm_spm_vb_source.m
...
% This script is an edited version of the SPM12 original spm_spm_vb.m script.
% This version was edited by Elise Rowe, July 2016.
% Please cite Bayesian Model Selection Maps for group studies using M/EEG 
% data by Elise G. Rowe, Clare D. Harris, Roshini Randeniya, and Marta I. Garrido
...
for s=1:nsess
    try
       sess_nScan = (SPM.Sess(s).row);
        %   sess_nScan = length(SPM.xX.K);
           sess_nScan = size(SPM.xX.X,1);

       if size(SPM.xX.K,2) > 0
        ...
        sprintf('No bandpass filter found\n')
    end
end
...
%-Remove small clusters - removes clusters containing < 0 voxels 
%-----------------------------------------------------------------------
...
for z = 1:nLb
    ...
    for s = 1:nsess
        ...
        %---------------------------------------------------------------
        R0Y = Y;
      %   R0Y = hpf(s).R0(:,1)*Y(SPM.Sess(s).row,:); %Need R0Y dim to be [4137, x]
